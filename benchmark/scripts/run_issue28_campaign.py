...
def run_campaign(contract_path: Path, output_root: Path, *, python: Path, dry_run: bool) -> int:
    ...
    for item in expected_units(contract):
        ...
        allowed = status == "success" and expected in {"success", "success_or_capacity_failure"}
        allowed = allowed or (status == "capacity_failure" and expected in {"success_or_capacity_failure", "capacity_failure"})
        if not dry_run and not allowed:
            overall_failures.append(f"{item['unit_id']}: status={status}, expected={expected}")
        _write_json(output_root / "campaign.json", {
            ...
            "units": units,
            "status": "planned" if dry_run else ("failed" if overall_failures else "running"),
